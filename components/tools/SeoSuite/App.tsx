...
const App: React.FC = () => {
  ...
  useAutoSave(
    ...
    {
      enabled: hasData && user !== null,
      interval: 60000,
      onSaveSuccess: () => console.log("SEO Suite session auto-saved")
    }
  );

  const handleRestore = (content: any) => {
    if (content.cannibalizationGroups) setCannibalizationGroups(content.cannibalizationGroups);
    if (content.comparisonRows) setComparisonRows(content.comparisonRows);
    if (content.siteStats) setSiteStats(content.siteStats);
    if (content.moduleStates) setModuleStates(content.moduleStates);
    if (content.autoTaskSession) setAutoTaskSession(content.autoTaskSession);
    if (content.selectedModel) setSelectedModel(content.selectedModel);
    if (content.providerConfig) setProviderConfig(content.providerConfig);
    if (content.externalKeys) setExternalKeys(content.externalKeys);

    setHasData(true);
    alert(lang === 'es' ? "Sesión restaurada." : "Session restored.");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-900 bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            ...
            {user && (
              <button
                ...
                className="ml-2 flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg text-xs font-bold transition-colors shadow-sm"
              >
      ...
      <HistoryModal
        ...
        resourceType="seo_suite_session"
