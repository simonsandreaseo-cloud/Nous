...
const ResearchReportModal = ({ isOpen, onClose, htmlReport }: { isOpen: boolean, onClose: () => void, htmlReport: string | null }) => {
    if (!isOpen || !htmlReport) return null;

    return (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.75)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
            <div style={{ backgroundColor: 'white', padding: '32px', borderRadius: '20px', width: '600px', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', border: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #F1F5F9', paddingBottom: '16px' }}>
                    <h3 style={{ margin: 0, fontSize: '20px', display: 'flex', alignItems: 'center', gap: '10px', fontFamily: 'Outfit, sans-serif', fontWeight: 800, color: '#4F46E5' }}>
                        <IconSparkles /> Informe de Investigación
                    </h3>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', fontSize: '24px' }}>&times;</button>
                </div>

                <div
                    style={{ fontSize: '14px', color: '#334155', lineHeight: '1.6', marginBottom: '24px' }}
                    dangerouslySetInnerHTML={{ __html: htmlReport }}
                />

                <div style={{ display: 'flex', justifyContent: 'end' }}>
                    <button
                        style={{ ...styles.button, backgroundColor: '#4F46E5', color: 'white', border: 'none', padding: '12px 24px', fontWeight: 700 } as any}
                        onClick={onClose}
                    >
                        Entendido, continuar
                    </button>
...
const App = () => {
    ...
    const handleResearchFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file) return;

        if (apiKeys.length === 0) {
            alert("Primero configura tus API Keys.");
            return;
        }

        setStatus("Leyendo archivo de investigación...");
        try {
            let text = "";
            ...
            setStatus("Analizando investigación con IA...");
            const analysis = await analyzeResearchContent(apiKeys, text);

            // AUTO-FILL DATA
            if (analysis.extractedData) {
                const ed = analysis.extractedData;
                if (ed.topic) setStrategyH1(ed.topic);
                if (ed.targetKeyword) setTargetKeyword(ed.targetKeyword);
                if (ed.tone) setStrategyTone(ed.tone);
                if (ed.wordCount) setStrategyWordCount(ed.wordCount.replace(/\D/g, '') || "1500");
                if (ed.niche) setDetectedNiche(ed.niche || "General");
                if (ed.competitors) setStrategyCompetitors(ed.competitors);

                if (ed.lsiKeywords && ed.lsiKeywords.length > 0) {
                    setStrategyLSI(ed.lsiKeywords.map(k => ({ keyword: k, count: "1" })));
                }
                if (ed.questions && ed.questions.length > 0) {
                    setStrategyQuestions(ed.questions);
                }
            }

            if (analysis.structure && analysis.structure.length > 0) {
                setStrategyOutline(analysis.structure);
            }

            // SHOW REPORT
            setResearchReport(analysis.report);
            setIsResearchReportOpen(true);
            setStatus("Investigación procesada.");

        } catch (error: any) {
            console.error(error);
            alert("Error al procesar investigación: " + error.message);
            setStatus("Error en investigación.");
        }
    };
                ...
                {viewMode === 'setup' && (
                    <div style={styles.hubContainer as any}>
                        <div style={{ ...styles.hubContent, maxWidth: '700px' } as any}>
                            ...
                            <div style={styles.stepCard as any}>
                                <div style={styles.stepTitle}>
                                    Subir Investigación (Opcional)
                                </div>
                                <p style={{ fontSize: '13px', color: '#64748B', marginBottom: '12px' }}>
                                    Sube un archivo DOCX, TXT o CSV con tu investigación previa. La IA extraerá automáticamente datos como palabras clave, estructura, competidores, etc.
                                </p>
                                <div
                                    style={{ ...styles.dropzone, borderStyle: 'dashed', borderColor: '#818CF8' } as any}
                                    onClick={() => researchInputRef.current?.click()}
                                >
                                    <input ref={researchInputRef} type="file" accept=".docx,.txt,.csv" style={{ display: 'none' }} onChange={handleResearchFile} />
                                    <IconUpload />
                                    <span style={{ fontSize: '14px', color: '#4F46E5', fontWeight: 600 }}>
                                        Cargar Documento de Investigación
                                    </span>
                                </div>
                            </div>
