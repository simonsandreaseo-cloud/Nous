...
export const generateRealImage = async (apiKeys: string[] | string, basePrompt: string, config: ImageGenConfig, context: 'featured' | 'body', aspectRatio: string = '16:9'): Promise<string> => {
    ...
    return executeWithKeyRotation(apiKeys, async (ai) => {
        try {
            const response = await ai.models.generateContent({
                model: 'gemini-2.0-flash', // Gemini doesn't have a separate -image model for generation yet in the same API call
                contents: {
...
export interface ResearchAnalysisResult {
    ...
    structure?: { type: string, text: string, wordCount: string, notes?: string }[];
    report: string;
}

export const analyzeResearchContent = async (apiKeys: string[] | string, fileContent: string): Promise<ResearchAnalysisResult> => {
    const truncatedContent = fileContent.length > 50000 ? fileContent.substring(0, 50000) + "...[truncated]" : fileContent;

    const prompt = `
    Analiza exhaustivamente este documento de investigación proporcionado por el usuario.
    Tu objetivo es extraer TODOS los datos posibles para rellenar la configuración de nuestra herramienta de redacción SEO.
    ...
    6. Nicho / Industria
    7. URLs de Competidores (si se mencionan, ponlas una por linea)
    8. Keywords LSI / Secundarias
    ...
    return executeWithKeyRotation(apiKeys, async (ai) => {
        ...
        try {
            ...
            console.error("Error parsing research analysis", e);
