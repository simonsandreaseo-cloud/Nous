...
interface DataConnectPanelProps {
    onAnalyze: (siteUrl: string, ga4PropertyId: string | null, startDateP1: string, endDateP1: string, startDateP2: string, endDateP2: string) => void;
    isLoading: boolean;
...
export const DataConnectPanel: React.FC<DataConnectPanelProps> = ({ onAnalyze, isLoading, initialSiteUrl }) => {
    ...
    const checkToken = async () => {
        ...
        try {
            // We use GSC service to check token validity, as it shares the same Google Auth
            const token = await GscService.getAccessToken();
            if (token) {
                setHasToken(true);
                await Promise.all([
                    loadGscSites(token),
                    loadGa4Properties(token)
                ]);
            } else {
                setHasToken(false);
            }
    ...
    const loadGscSites = async (token: string) => {
        try {
            ...
            console.error("Failed to load GSC sites", e);
        }
    };
    ...
    const handleAnalyzeClick = () => {
        if (!selectedSite) return alert("Selecciona una propiedad de Search Console.");

        const ga4Id = selectedGa4Property || null;

        if (compareMode === 'day') {
            onAnalyze(selectedSite, ga4Id, dayP1, dayP1, dayP2, dayP2);
        } else {
            onAnalyze(selectedSite, ga4Id, dateRangeP1.start, dateRangeP1.end, dateRangeP2.start, dateRangeP2.end);
        }
    };
    ...
    if (hasToken === false) {
        return (
            <div className="text-center py-16 bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2 tracking-tight">Conexión con Datos</h3>
                <p className="text-slate-500 mb-8 max-w-sm mx-auto leading-relaxed">Necesitamos acceso a tus cuentas de Google (Search Console y Analytics) para la inteligencia de datos.</p>
                <button
    ...
    if (hasToken === null || loadingResources) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-6">
                ...
                <div className="text-center">
                    <p className="text-indigo-600 font-extrabold tracking-wide uppercase text-xs mb-1">Cargando orígenes de datos</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                    <label className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-widest flex items-center gap-2">
                        <img src="https://www.gstatic.com/images/branding/product/1x/search_console_48dp.png" className="w-4 h-4" alt="GSC" />
                        PROPIEDAD SEARCH CONSOLE
                    </label>
                    <select
                        value={selectedSite}
                        onChange={(e) => setSelectedSite(e.target.value)}
                        className="w-full text-sm p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none font-bold text-slate-800 transition-all appearance-none cursor-pointer"
                    >
                        {sites.map(s => (
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <label className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-widest flex items-center gap-2 relative z-10">
                        <img src="https://www.gstatic.com/analytics-suite/header/analytics_24px.png" className="w-4 h-4" alt="GA4" />
                        PROPIEDAD GA4 (Opcional)
                    </label>
                    ...
                    {ga4Properties.length > 0 ? (
                        <select
                            ...
                            className="w-full text-sm p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none font-bold text-slate-800 transition-all appearance-none cursor-pointer relative z-10"
                        >
                            <option value="">-- Sin conexión GA4 --</option>
                            {ga4Properties.map(p => (
