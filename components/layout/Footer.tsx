      ...
      <div className="max-w-screen-2xl mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-end gap-10">
        ...
        <div className="space-y-6">
          ...
          <p className="text-brand-power/30 text-xs mt-12 font-mono uppercase tracking-widest">
            © {new Date().getFullYear()} {SITE_CONFIG.author}. Madrid.
          </p>
        </div>

        {/* Socials - Low Contrast per Blue Paper strategy 4.3 */}
        <nav className="flex gap-8" aria-label="Redes Sociales">
          {['LinkedIn', 'Twitter', 'GitHub'].map((social) => (
            <a 
              key={social} 
              href="#" 
              className="text-brand-power/30 hover:text-brand-power text-xs font-mono uppercase tracking-widest transition-colors duration-300"
              aria-label={`Visitar perfil de ${social}`}
            >
              {social}
            </a>
          ))}
