...
const sql = `
CREATE OR REPLACE FUNCTION public.get_semantic_inventory_matches_v3(p_project_id uuid, p_base_regex text, p_ask_regex text, p_limit integer DEFAULT 100)
...
DECLARE
  ...
  v_prefilter_regex text := '';
BEGIN
  IF p_base_regex IS NOT NULL AND p_base_regex != '' THEN
    SELECT array_agg(term) INTO v_base_terms FROM (
      SELECT trim(word) as term FROM regexp_split_to_table(p_base_regex, '\\|') as word WHERE length(trim(word)) > 2
    ) t;
  END IF;
  
  IF p_ask_regex IS NOT NULL AND p_ask_regex != '' THEN
    SELECT array_agg(term) INTO v_ask_terms FROM (
      SELECT trim(word) as term FROM regexp_split_to_table(p_ask_regex, '\\|') as word WHERE length(trim(word)) > 2
    ) t;
  END IF;

  IF array_length(v_base_terms, 1) > 0 THEN
    v_prefilter_regex := '(' || array_to_string(v_base_terms[1:LEAST(array_upper(v_base_terms, 1), 10)], ')|(') || ')';
  ELSIF array_length(v_ask_terms, 1) > 0 THEN
    v_prefilter_regex := '(' || array_to_string(v_ask_terms[1:LEAST(array_upper(v_ask_terms, 1), 10)], ')|(') || ')';
  END IF;

  IF v_prefilter_regex != '' THEN
    RETURN QUERY
    SELECT 
      pu.url,
    ...
    WHERE pu.project_id = p_project_id
      AND (
        (v_prefilter_regex != '' AND (pu.url ~* v_prefilter_regex OR COALESCE(pu.title, '') ~* v_prefilter_regex))
        OR 
        (array_length(v_ask_terms, 1) > 0 AND (
          SELECT count(*) FROM unnest(v_ask_terms) as term WHERE pu.url ~* term OR COALESCE(pu.title, '') ~* term
        ) > 0)
      )
  ...
  ELSE
    ...
    FROM project_urls pu
    WHERE pu.project_id = p_project_id
    ORDER BY pu.impressions_gsc DESC NULLS LAST
    LIMIT p_limit;
  END IF;
END;
$function$;
