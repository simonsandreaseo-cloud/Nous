import { Project, SyntaxKind, ArrowFunction, CallExpression, PropertyAccessExpression } from 'ts-morph';
...
for (const call of enqueueCalls) {
    ...
    if (args.length >= 3 && args[2].getKind() === SyntaxKind.ArrowFunction) {
        ...
        // Find all state mutations inside the arrow function
        ...
        for (const mut of mutations) {
            ...
            if (statement && !statement.getText().startsWith('if')) {
                statement.replaceWithText(\if (useWriterStore.getState().draftId === targetTaskId) { \ }\);
            }
        }
...
sourceFile.saveSync();
console.log('AST manipulation complete Part 2');
