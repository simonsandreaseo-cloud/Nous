...
import { runCustomTransformPipeline } from '@/lib/actions/aiActions';
...
export async function POST(req: Request) {
    try {
        ...
        const { content, presetInstructions, userInstructions, model, provider } = body;

        if (!content) {
            return NextResponse.json({ error: 'Content is required' }, { status: 400 });
        }

        console.log(`[CustomTransform-API] Processing ${content.length} chars (Streaming)`);
