...
import { handleSurgicalEditTask } from './surgicalEdit';
...
vi.mock('@/store/useWriterStore', () => ({
  useWriterStore: {
    getState: vi.fn().mockReturnValue({
      draftId: 'test-draft',
      content: '',
      setSurgicalEditing: vi.fn(),
      setSurgicalEditStatus: vi.fn(),
      setContent: vi.fn(),
...
describe('Surgical Edit Queue Handler Protection Integration', () => {
  const mockPayload = {
    taskId: 'test-task',
  ...
  it('should follow the Protect -> Chunk -> AI -> Restore flow', async () => {
    ...
    await handleSurgicalEditTask('test-task', mockPayload as any);
