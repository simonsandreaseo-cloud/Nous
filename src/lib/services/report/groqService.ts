...
async function queryAI(prompt: string, apiKey: string, modelId: string = 'quality', jsonResponse: boolean = false): Promise<string> {
    ...
    useWriterStore.getState().addDebugPrompt(`Groq Query: ${modelId}`, prompt, responseText);
...
[Data]: ${safePayload}`;
    ...
    try {
        ...
        console.warn(`[GROQ-SERVICE] JSON Section ${sectionKey} failed with ${modelId}:`, e.message);
        throw e;
    }
}
...
export const generateJSONReportState = async (payload: ReportPayload, sections: string[], apiKey: string): Promise<any[]> => {
    ...
    allSlides.push({
        ...
        subtitle: "Estrategia, análisis y rendimiento propulsado por Groq.",
        date: new Date().toLocaleDateString('es-ES', { month: 'long', year: 'numeric' }).toUpperCase()
    });

    console.log(`[GROQ-SERVICE] Starting Chunked JSON Generation for ${sections.length} sections.`);
    ...
    for (const section of sections) {
        ...
        for (const modelId of models) {
            try {
                ...
                console.warn(`[GROQ-SERVICE] Retrying section ${section} with quality model...`);
            }
        }
...
export const generateContent = async (prompt: string, context: string = '', apiKey: string): Promise<string> => {
    ...
    try {
        const text = await queryAI(fullPrompt, apiKey, 'quality');
        // Strip markdown code blocks if present
        return text.replace(/```html/g, '').replace(/```/g, '').trim();
    } catch (e: any) {
        console.warn("[GROQ-SERVICE] generateContent failed:", e.message);
...
export const generateInsightAnalysis = async (context: string, apiKey: string): Promise<string> => {
    ...
    try {
        ...
        console.warn("[GROQ-SERVICE] generateInsightAnalysis failed:", e.message);
