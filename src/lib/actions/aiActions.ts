...
export const runHumanizerPipeline = async (
    ...
    try {
        const processedHtml = await executeHumanizerWithRetry(async (ai) => {
            ...
            const prompt = `--- TAREA ---\nAplica TODAS las reglas de humanización al texto DENTRO de las etiquetas HTML del siguiente bloque. NUNCA modifiques los textos de los encabezados (h1, h2, h3, etc.).\n\nPROHIBICIÓN ESTRICTA: CERO RAZONAMIENTO. Devuelve SOLO el bloque \`\`\`html final.\n\n--- HTML DE ENTRADA ---\n\`\`\`html\n${protectedHtml}\n\`\`\`\n${languageInstruction}`;
