...
import { MOCK_ARTICLES, Article, ArticleTool } from "./ArticleCardGrid";
...
const STATUS_COLORS: Record<string, string> = {
    ...
    "in-progress": "text-[var(--color-nous-lavender)] bg-[var(--color-nous-lavender)]/10",
...
export function ArticleTable() {
    ...
    const toggleAll = () => {
        if (selected.size === MOCK_ARTICLES.length) setSelected(new Set());
        else setSelected(new Set(MOCK_ARTICLES.map(a => a.id)));
    };
    ...
    const sorted = [...MOCK_ARTICLES].sort((a, b) => {
        ...
        <div className="flex flex-col gap-3">
            ...
            {selected.size > 0 && (
                <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-900 text-white rounded-2xl">
                    ...
                    <button className="flex items-center gap-1.5 text-[10px] font-black tracking-widest uppercase hover:text-slate-300 transition-colors">
                        <Send size={11} />Enviar a herramienta
                    </button>
                    <button className="flex items-center gap-1.5 text-[10px] font-black tracking-widest uppercase text-red-400 hover:text-red-300 transition-colors">
                    ...
                    <thead>
                        <tr className="border-b border-slate-100">
                            <th className="w-10 px-4 py-3">
                                <input
                                    type="checkbox"
                                    checked={selected.size === MOCK_ARTICLES.length}
                                    onChange={toggleAll}
