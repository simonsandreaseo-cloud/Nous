...
export default function WriterSetupBoard() {
    ...
    const handleInvestigacionNous = async () => {
        ...
        try {
            ...
            const seoData = await runDeepSEOAnalysis(store.keyword, store.csvData, store.projectName, false, activeProject?.id);
    ...
    const handleFinish = async () => {
        if (!store.draftId) {
            try {
                ...
                store.setStatus("Creando borrador...");
                const { data: userData } = await supabase.auth.getUser();
                
                if (!userData?.user?.id) {
                     alert("Sesión expirada. Trabajando en modo local.");
                ...
                const { data: newContent, error } = await supabase
                    ...
                    .insert({
                        ...
                        status: 'drafting',
                        content_body: '',
                        seo_data: store.rawSeoData,
                        // author_id: userData.user.id // Hecho opcional según solicitud
                    }).select().single();
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50/50">
            ...
            <div className="px-6 py-5 border-b border-slate-100 bg-white/95 backdrop-blur-xl flex items-center justify-between shadow-sm z-20 shrink-0">
                ...
                <button onClick={() => store.setViewMode('dashboard')} className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-800 transition-colors">
                    <X size={16} />
                </button>
            </div>
