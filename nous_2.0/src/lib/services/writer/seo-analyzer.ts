...
export const runSEOAnalysis = async (keyword: string, onLog?: (p: string, pr: string, res?: string) => void): Promise<SEOAnalysisResult> => {
    ...
    if (onLog) onLog("Query Google", smartQuery, "N/A");
    let rawResults = (await fetchSerperSearch(smartQuery)).map(r => ({ ...r, url: r.link || r.url }));
    if (!rawResults || rawResults.length < 5) {
        ...
        const fallbackResults = (await fetchSerperSearch(keyword)).map(r => ({ ...r, url: r.link || r.url }));
        if (fallbackResults.length > rawResults.length) rawResults = fallbackResults;
    }
    ...
    return { ...aiAnalysis, top10Urls: filtered.slice(0, 8), top20Urls: filtered };
};

export const runDeepSEOAnalysis = async (config: DeepSEOConfig) => {
    const { keyword, jinaKey, projectId, taskId, onProgress, onLog, modelName = 'gemini-2.5-flash', isFastMode = false } = config;
    ...
    if (isFastMode) {
        ...
        (baseResult.top10Urls || []).forEach(comp => { scrapedSEO.push({ url: comp.url, title: comp.title, content: comp.snippet || "", summary: comp.snippet || "" }); });
    } else {
        if (onProgress) onProgress("scraping");
        ...
        if (competitors.length > 0) {
            const scrapeResults = await Promise.allSettled(competitors.slice(0, 8).map(async (comp, idx) => {
                if (!comp.url || !comp.url.startsWith('http')) return { url: comp.url, title: comp.title, isInvalid: true } as any;
                try {
                    ...
                    return { url: comp.url, title: comp.title, content: data.content, summary: data.title + ": " + data.content?.substring(0, 300) };
                } catch (innerE: any) {
                    if (onLog) onLog(`Descarte ${idx + 1}`, `${comp.url.substring(0, 30)}`, innerE.message);
                    return { url: comp.url, title: comp.title, content: comp.snippet || "", summary: `(Fallback Snippet) ${comp.snippet}`, isPartial: true } as any;
                }
            }));
            scrapeResults.forEach(res => { if (res.status === 'fulfilled') scrapedSEO.push(res.value); });
        }
    }
    ...
    if (projectId) {
        try {
            ...
            if (combinedPool.length > 0) suggestedLinks = await selectSemanticInternalLinks(keyword, combinedPool, 15, onLog, JSON.stringify({ metadata: seoMetadata, lsi: cleanedLSI }), modelName);
        } catch (e) {}
    }
    ...
    const finalResult = { ...baseResult, ...seoMetadata, lsiKeywords: cleanedLSI, suggestedInternalLinks: suggestedLinks, research_dossier: { ...baseResult, lsiKeywords: cleanedLSI, suggestedInternalLinks: suggestedLinks, seoMetadata }, status: "por_redactar" };
    if (taskId) {
